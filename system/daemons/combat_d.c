 ...
 * @LastEditTime: 2023-05-19 11:46:03
...
void heart_beat( void )
{
    if(fight_time >= 0)
        fight_time += 1;  //时间加1s
    if(combat->length_fight_info())
        combat->disable_user_command();
    else
        combat->enable_user_command();
...
int put_card_d(int *n_card,object user)
{
    ...
    if(n_card == 0)
    {
        o->put_card_d_pass(combat,script);
        return 1;
    }
    o->put_card(combat,n_card,user,script);
    return 1;
}
//获取user手牌数量
